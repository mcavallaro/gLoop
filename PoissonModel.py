...
def Usage():
    ...
    print('If using Python > 3.3, the launch the script with: >> PYTHONHASHSEED=0 parallel python3 PoissonModel.py ::: ...')
    sys.exit()
...
def main(data, mean_mu, var_mu, mu_BKG, sd_BKG, alpha_BKG, n_steps, folder, db_file_name):
 """
  ...
  X = pymc.Container(
    [pymc.Poisson('X%i'%i, mu=Mean, value=init_X[i], trace=trace_[i]) for i in range(n_blocks)],
    )
  ...
  if db_file_name is None:
    MCMC = pymc.MCMC([Mean, Kappa, X, Y, Y_Sim], db='hdf5', dbname=path + '/Poisson_flow-FISH.h5')
    MCMC.use_step_method(pymc.AdaptiveMetropolis, [Mean, Kappa], shrink_if_necessary=True)
    ...
    MCMC = pymc.MCMC([Mean, Kappa, X, Y, Y_Sim], db=db)

    corr = MCMC.db.getstate()['step_methods']['AdaptiveMetropolis_K_Mean_Kappa']['proposal_sd'] ##
    cov = np.dot(corr, corr.T)
    MCMC.use_step_method(pymc.AdaptiveMetropolis, [Mean, Kappa], shrink_if_necessary=False, cov=cov)
